...
class ConfigService(BaseSettings):
    ...
    @classmethod
    def make_database_conn(cls, v, values): # pylint: disable=unused-argument
        value = values.data['database_dsn']
        return dj_database_url.config(default=value)

    @field_validator('installed_apps', mode='before')
    @classmethod
    def split_installed_apps(cls, v, values): # pylint: disable=unused-argument
    ...
    @classmethod
    def split_middlewares_additional(cls, v, values): # pylint: disable=unused-argument
