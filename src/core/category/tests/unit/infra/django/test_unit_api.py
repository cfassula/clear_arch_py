...
class TestCategoryResourceUnit(unittest.TestCase):
    ...
    def test_validate_id_method(self, mock_serializer):
        ...
        mock_serializer.return_value = namedtuple(
            'Faker', ['is_valid'])(is_valid=mock_serializer_is_valid)
        CategoryResource.validate_id('invalid id')
        mock_serializer.assert_called_with(
            UUIDSerializer,
            data={'id': 'invalid id'}
        )
    ...
    def test_post_method(self, mock_category_to_response):
        ...
        send_data = {'name': 'test'}
        expected_response = {
            'id': 'af468b2c-8f4b-4e8d-8e0b-8b4c4a3c1b8c',
            'name': 'test',
            'description': None,
            'is_active': True,
        ...
        with mock.patch.object(
            ...
            stub_serializer.is_valid = mock.Mock()
    ...
    def test_get_method(self):
        ...
        mock_list_use_case.execute.assert_called_once_with(ListCategoriesUseCase.Input(
            page='1',
            per_page='1',
            sort='name',
            sort_dir='asc',
    ...
    def test_put_method(self, mock_validate_id, mock_category_to_response):
        ...
        request = make_request(http_method='put', send_data=send_data)
    ...
    def test_delete_method(self, mock_validate_id):
        ...
        mock_validate_id.assert_called_with(uuid_value)
        mock_delete_use_case.execute.assert_called_once_with(DeleteCategoryUseCase.Input(
            id=uuid_value
        ))
