...
class TestCreateCategoryUseCaseUnit(unittest.TestCase):
    ''' Test CreateCategoryUseCase Unit '''
    ...
    def test_output(self):
        ''' Test Output class '''
        self.assertTrue(
            issubclass(CreateCategoryUseCase.Output,
                        CategoryOutput
            )
        )
...
class TestGetCategoryUseCaseUnit(unittest.TestCase):
    ...
    def test_output(self):
        ''' Test Output class '''
        self.assertTrue(
            issubclass(GetCategoryUseCase.Output,
                       CategoryOutput
                       )
        )

    def test_throw_exception_when_category_not_found(self):
        ''' Test throw exception when category not found '''
        input_param = GetCategoryUseCase.Input('invalid-id')
        with self.assertRaises(NotFoundException) as assert_error:
            self.use_case.execute(input_param)
        self.assertEqual(
            str(assert_error.exception.args[0]),
            "Entity not found using ID 'invalid-id'"
        )
...
class TestListCategoriesUseCase(unittest.TestCase):
    ...
    def test_execute_using_empty_search_params(self):
        self.category_repo.items = [
            ...
            Category(name='test 2', created_at=datetime.datetime.now(datetime.timezone.utc) +
