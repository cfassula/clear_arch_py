...
class CategoryFakerBuilder(Generic[T]):
    ...
    def with_invalid_name_too_long(self, value: Any = None):
    ...
    def build(self) -> T:
        categories = list(
            map(
                ...
                list(range(self.count_objects))
            )

        )
        return categories if self.count_objects > 1 else categories[0]
    ...
    def __call_factory(self, value: PropOrFactory[T], index: int) -> T:
